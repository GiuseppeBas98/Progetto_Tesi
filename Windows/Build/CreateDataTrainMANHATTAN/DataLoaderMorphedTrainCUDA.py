...
def graph2Data(graph, type):
    ...
    x = torch.tensor(normalized_landmark_coordinates, dtype=torch.float32, device=torch.device('cuda'))  # Tensor x with normalized coordinates
    # print(f'Shape of tensor x: {x.shape}, Node Features Tensor: {x}')

    # EDGES
    edges = list(graph.edges)
    edge_index = torch.tensor(edges, dtype=torch.long, device=torch.device('cuda')).t().contiguous() # Tensor of graph edges
    ...
    edge_weights = torch.tensor(edge_weights_normalized, dtype=torch.float32, device=torch.device('cuda'))  # Sposta il tensore sulla GPU, Tensor of edge weights/distances
    ...
    if type == 'morphed':
        ...
        y = 1
    y = torch.tensor(y, dtype=torch.long, device=torch.device('cuda'))
    # print(y)
    # print(f'y: {y}')
    ...
    # DATA
    graph = data.Data(x=x, edge_index=edge_index, edge_weight=edge_weights, y=y).to(torch.device('cuda'))
    # print(graph)
...
def crea_dataLoader():
    global array
    path_dataloader_daEliminare = r"C:\Users\Giuseppe Basile\Desktop\New_Morphing\dataloaders\TrainDataloader_64SizeCUDA.pt"

    # Verifica se il file esiste
    if os.path.exists(path_dataloader_daEliminare):
        # Se il file esiste, sovrascrivi il dataloader e salva
        data_loader = load_dataloader('TrainDataloader_64SizeCUDA')
        ...
        dataLoader = create_dataloader(array, batch_size=64)
        save_dataloader(dataLoader, 'TrainDataloader_64SizeCUDA')
        ...
        data_loader = create_dataloader(array, batch_size=64)
        save_dataloader(data_loader, 'TrainDataloader_64SizeCUDA')
...
def main():
    ...
    d = load_dataloader('TrainDataloader_64SizeCUDA')
    # d1 = load_dataloader('TestDataloadermorph_opencv')
    dset = d.dataset
    # dset1 = d1.dataset
    print(str(len(dset)) + ' spazio ' + str(len(d)))
